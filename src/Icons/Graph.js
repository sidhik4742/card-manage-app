import React from 'react';
import {StyleSheet, Text, View} from 'react-native';
import Svg, {Path, G} from 'react-native-svg';

const Graph = () => {
  return (
    <Svg
      id="Capa_1"
      enableBackground="new 0 0 512.001 512.001"
      height={30}
      viewBox="0 0 512.001 512.001"
      width={30}
      xmlns="http://www.w3.org/2000/svg"
    >
      <G>
        <G>
          <Path
            d="m105.376 511.001h-60.25c-8.284 0-15-6.716-15-15v-180.75c0-8.284 6.716-15 15-15h60.25c8.284 0 15 6.716 15 15v180.75c0 8.284-6.716 15-15 15z"
            fill="#ffdd6c"
          />
        </G>
        <G>
          <Path
            d="m225.876 511.001h-60.25c-8.284 0-15-6.716-15-15v-241c0-8.284 6.716-15 15-15h60.25c8.284 0 15 6.716 15 15v241c0 8.284-6.716 15-15 15z"
            fill="#f77600"
          />
        </G>
        <G>
          <Path
            d="m346.376 511.001h-60.25c-8.284 0-15-6.716-15-15v-301.25c0-8.284 6.716-15 15-15h60.25c8.284 0 15 6.716 15 15v301.25c0 8.284-6.716 15-15 15z"
            fill="#1fb4ff"
          />
        </G>
        <G>
          <Path
            d="m466.876 511.001h-60.25c-8.284 0-15-6.716-15-15v-361.5c0-8.284 6.716-15 15-15h60.25c8.284 0 15 6.716 15 15v361.5c0 8.284-6.716 15-15 15z"
            fill="#ff4c5b"
          />
        </G>
        <Path
          d="m105.376 300.251h-30.125v210.75h30.125c8.284 0 15-6.716 15-15v-180.75c0-8.285-6.716-15-15-15z"
          fill="#fec300"
        />
        <Path
          d="m225.876 240.001h-30.125v271h30.125c8.284 0 15-6.716 15-15v-241c0-8.285-6.716-15-15-15z"
          fill="#ff4c5b"
        />
        <Path
          d="m346.376 179.751h-30.125v331.25h30.125c8.284 0 15-6.716 15-15v-301.25c0-8.285-6.716-15-15-15z"
          fill="#2982ff"
        />
        <Path
          d="m466.876 119.501h-30.125v391.5h30.125c8.284 0 15-6.716 15-15v-361.5c0-8.285-6.716-15-15-15z"
          fill="#d90048"
        />
        <G>
          <Path
            d="m497.001 512.001h-482c-8.284 0-15-6.716-15-15s6.716-15 15-15h482c8.284 0 15 6.716 15 15s-6.716 15-15 15z"
            fill="#384949"
          />
        </G>
        <Path
          d="m466.876.001h-60.25c-8.284 0-15 6.716-15 15s6.716 15 15 15h23.236c-16.167 13.787-40.426 32.025-73.849 51.071-63.828 36.373-175.058 83.205-342.486 99.751-8.244.815-14.267 8.158-13.452 16.402.765 7.743 7.289 13.526 14.909 13.526.493 0 .992-.024 1.493-.074 76.631-7.574 148.46-21.938 213.49-42.696 52.234-16.673 100.248-37.492 142.708-61.88 36.106-20.738 62.038-40.508 79.201-55.404v24.553c0 8.284 6.716 15 15 15s15-6.716 15-15v-60.25c0-8.284-6.716-14.999-15-14.999z"
          fill="#d9e9fa"
        />
        <Path
          d="m497.001 482.001h-240.999v30h240.999c8.284 0 15-6.716 15-15s-6.716-15-15-15z"
          fill="#151f1f"
        />
        <Path
          d="m466.876.001h-60.25c-8.284 0-15 6.716-15 15s6.716 15 15 15h23.236c-16.167 13.787-40.426 32.025-73.849 51.071-25.55 14.56-58.705 30.795-100.01 46.093v32.025c42.216-15.011 81.369-32.811 116.673-53.088 36.106-20.738 62.038-40.508 79.201-55.404v24.553c0 8.284 6.716 15 15 15s15-6.716 15-15v-60.25c-.001-8.285-6.717-15-15.001-15z"
          fill="#bfd9f5"
        />
      </G>
    </Svg>
  );
};

const styles = StyleSheet.create({});
export default Graph;
